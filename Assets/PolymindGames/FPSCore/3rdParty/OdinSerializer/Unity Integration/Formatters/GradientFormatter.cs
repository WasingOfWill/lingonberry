...
namespace PolymindGames.OdinSerializer
{
    ...
    public sealed class GradientFormatter : MinimalBaseFormatter<Gradient>
    {
        private static readonly Serializer<GradientAlphaKey[]> s_AlphaKeysSerializer = Serializer.Get<GradientAlphaKey[]>();
        private static readonly Serializer<GradientColorKey[]> s_ColorKeysSerializer = Serializer.Get<GradientColorKey[]>();
        ...
        private static readonly PropertyInfo s_ModeProperty = typeof(Gradient).GetProperty("mode", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
        private static readonly Serializer<object> s_EnumSerializer = s_ModeProperty != null ? Serializer.Get<object>() : null;
        ...
        {
            value.alphaKeys = s_AlphaKeysSerializer.ReadValue(reader);
            value.colorKeys = s_ColorKeysSerializer.ReadValue(reader);

            reader.PeekEntry(out string name);

            if (name == "mode")
            {
                try
                {
                    if (s_ModeProperty != null)
                    {
                        s_ModeProperty.SetValue(value, s_EnumSerializer.ReadValue(reader), null);
                    }
                    else
                    {
                        reader.SkipEntry();
        ...
        {
            s_AlphaKeysSerializer.WriteValue(value.alphaKeys, writer);
            s_ColorKeysSerializer.WriteValue(value.colorKeys, writer);

            if (s_ModeProperty != null)
            {
                try
                {
                    s_EnumSerializer.WriteValue("mode", s_ModeProperty.GetValue(value, null), writer);
                }
                catch (Exception)
                {
