...
namespace PolymindGames.OdinSerializer
{
    ...
    {
        private static readonly Serializer<TPrimary> s_PrimaryReaderWriter = Serializer.Get<TPrimary>();
        private static readonly Serializer<Dictionary<TSecondary, TValue>> s_InnerReaderWriter = Serializer.Get<Dictionary<TSecondary, TValue>>();
            ...
            try
            {
                ...
                foreach (var pair in value)
                {
                    try
                    {
                        ...
                        s_PrimaryReaderWriter.WriteValue("$k", pair.Key, writer);
                        s_InnerReaderWriter.WriteValue("$v", pair.Value, writer);
                    }
                ...
                try
                {
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        try
                        {
                            ...
                            TPrimary key = s_PrimaryReaderWriter.ReadValue(reader);
                            Dictionary<TSecondary, TValue> inner = s_InnerReaderWriter.ReadValue(reader);
