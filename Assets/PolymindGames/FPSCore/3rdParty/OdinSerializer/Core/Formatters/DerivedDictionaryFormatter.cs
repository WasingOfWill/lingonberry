...
namespace PolymindGames.OdinSerializer
{
    ...
    {
        private static readonly bool s_KeyIsValueType = typeof(TKey).IsValueType;

        private static readonly Serializer<IEqualityComparer<TKey>> s_EqualityComparerSerializer = Serializer.Get<IEqualityComparer<TKey>>();
        private static readonly Serializer<TKey> s_KeyReaderWriter = Serializer.Get<TKey>();
        private static readonly Serializer<TValue> s_ValueReaderWriter = Serializer.Get<TValue>();

        private static readonly ConstructorInfo s_ComparerConstructor = typeof(TDictionary).GetConstructor(new[] { typeof(IEqualityComparer<TKey>) });
        ...
        {
            var entry = reader.PeekEntry(out string name);
            ...
            if (name == "comparer" || entry == EntryType.StartOfNode)
            {
                ...
                comparer = s_EqualityComparerSerializer.ReadValue(reader);
            ...
            if (entry == EntryType.StartOfArray)
            {
                try
                {
                    ...
                    if (!ReferenceEquals(comparer, null) && s_ComparerConstructor != null)
                    {
                        value = (TDictionary)s_ComparerConstructor.Invoke(new object[] { comparer });
                    }
                    else
                    {
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        try
                        {
                            ...
                            TKey key = s_KeyReaderWriter.ReadValue(reader);
                            TValue val = s_ValueReaderWriter.ReadValue(reader);

                            if (!s_KeyIsValueType && ReferenceEquals(key, null))
                            {
        ...
        protected override void SerializeImplementation(ref TDictionary value, IDataWriter writer)
        {
            try
            {
                ...
                {
                    s_EqualityComparerSerializer.WriteValue("comparer", value.Comparer, writer);
                }
                ...
                foreach (var pair in value)
                {
                    ...
                    try
                    {
                        ...
                        s_KeyReaderWriter.WriteValue("$k", pair.Key, writer);
                        s_ValueReaderWriter.WriteValue("$v", pair.Value, writer);
                    }
                    catch (SerializationAbortException ex)
                    {
                        endNode = false;
