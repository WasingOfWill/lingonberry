...
namespace PolymindGames.OdinSerializer
{
    ...
    public static class FormatterEmitter
    {
        ...
        /// </summary>
        private static int s_HelperFormatterNameId;
        ...
        private static readonly DoubleLookupDictionary<ISerializationPolicy, Type, IFormatter> s_Formatters = new();

        private static AssemblyBuilder s_RuntimeEmittedAssembly;
        private static ModuleBuilder s_RuntimeEmittedModule;
        ...
        return null;
            ...
            if (s_Formatters.TryGetInnerValue(policy, type, out var result) == false)
            {
                lock (s_Lock)
                {
                    if (s_Formatters.TryGetInnerValue(policy, type, out result) == false)
                    {
                        EnsureRuntimeAssembly();

                        try
                        {
                            result = CreateGenericFormatter(type, s_RuntimeEmittedModule, policy);
                        }
                        catch (Exception ex)
                        {
                        ...
                        s_Formatters.AddInner(policy, type, result);
                    }
                }
            }
        ...
        private static void EnsureRuntimeAssembly()
        {
            ...
            if (s_RuntimeEmittedAssembly == null)
            {
                ...
                s_RuntimeEmittedAssembly = AppDomain.CurrentDomain.DefineDynamicAssembly(assemblyName, AssemblyBuilderAccess.Run);
            }

            if (s_RuntimeEmittedModule == null)
            {
                bool emitSymbolInfo;
                ...
                s_RuntimeEmittedModule = s_RuntimeEmittedAssembly.DefineDynamicModule(RUNTIME_EMITTED_ASSEMBLY_NAME, emitSymbolInfo);
            }
        }
        ...
        public static Type EmitAOTFormatter(Type formattedType, ModuleBuilder moduleBuilder, ISerializationPolicy policy)
        {
            ...
            BuildHelperType(
                ...
                formattedType,
                serializableMembers,
                out var serializerReadMethods,
                out var serializerWriteMethods,
                out var serializerFields,
                out var dictField,
                out var memberNames
            );
        ...
        private static IFormatter CreateGenericFormatter(Type formattedType, ModuleBuilder moduleBuilder, ISerializationPolicy policy)
        {
            ...
            string helperTypeName = moduleBuilder.Name + "." + 
                ...
                System.Threading.Interlocked.Increment(ref s_HelperFormatterNameId);
            ...
            out Dictionary<MemberInfo, List<string>> memberNames)
        {
            ...
            foreach (var entry in serializableMembers)
            {
                ...
                if (memberNames.TryGetValue(entry.Value, out var list) == false)
                {
