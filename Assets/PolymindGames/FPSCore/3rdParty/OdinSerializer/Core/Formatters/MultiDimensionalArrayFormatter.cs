...
namespace PolymindGames.OdinSerializer
{
    ...
    public sealed class MultiDimensionalArrayFormatter<TArray, TElement> : BaseFormatter<TArray> where TArray : class
    {
        ...
        private static readonly int s_ArrayRank;
        private static readonly Serializer<TElement> s_ValueReaderWriter = Serializer.Get<TElement>();
        ...
        static MultiDimensionalArrayFormatter()
        {
            ...
            s_ArrayRank = typeof(TArray).GetArrayRank();
        ...
        protected override void DeserializeImplementation(ref TArray value, IDataReader reader)
        {
            ...
            if (entry == EntryType.StartOfArray)
            {
                ...
                reader.ReadString(out string lengthStr);

                string[] lengthsStrs = lengthStr.Split(RANKS_SEPARATOR);

                if (lengthsStrs.Length != s_ArrayRank)
                {
                    value = default(TArray);
                    reader.SkipEntry();
                    return;
                ...
                {
                    if (int.TryParse(lengthsStrs[i], out int rankVal))
                    {
                        lengths[i] = rankVal;
                    }
                    else
                ...
                try
                {
                    IterateArrayWrite(
                        ...
                        () =>
                        {
                            ...
                            var v = s_ValueReaderWriter.ReadValue(reader);
        ...
        protected override void SerializeImplementation(ref TArray value, IDataWriter writer)
        {
            ...
            try
            {
                ...
                int[] lengths = new int[s_ArrayRank];

                for (int i = 0; i < s_ArrayRank; i++)
                {
                ...
                IterateArrayRead(
                    ...
                    (v) =>
                    {
                        s_ValueReaderWriter.WriteValue(v, writer);
                    });
            }
            finally
            {
        ...
        {
            int[] indices = new int[s_ArrayRank];
