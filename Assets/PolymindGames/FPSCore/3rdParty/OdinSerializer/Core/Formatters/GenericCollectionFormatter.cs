...
namespace PolymindGames.OdinSerializer
{
    ...
    {
        private static Serializer<TElement> s_ValueReaderWriter = Serializer.Get<TElement>();

        static GenericCollectionFormatter()
        {

            if (GenericCollectionFormatter.CanFormat(typeof(TCollection), out var e) == false)
            {
            ...
            if (entry == EntryType.StartOfArray)
            {
                try
                {
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        try
                        {
                            value.Add(s_ValueReaderWriter.ReadValue(reader));
                        }
                        catch (Exception ex)
                        {
        ...
        protected override void SerializeImplementation(ref TCollection value, IDataWriter writer)
        {
            try
            {
                ...
                {
                    s_ValueReaderWriter.WriteValue(element, writer);
                }
            }
            finally
            {
