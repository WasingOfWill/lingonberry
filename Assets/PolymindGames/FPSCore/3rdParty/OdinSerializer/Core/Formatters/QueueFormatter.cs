...
namespace PolymindGames.OdinSerializer
{
    ...
    public sealed class QueueFormatter<TQueue, TValue> : BaseFormatter<TQueue>
        where TQueue : Queue<TValue>, new()
    {
        private static readonly Serializer<TValue> s_TSerializer = Serializer.Get<TValue>();
        private static readonly bool s_IsPlainQueue = typeof(TQueue) == typeof(Queue<TValue>);
            ...
            if (entry == EntryType.StartOfArray)
            {
                try
                {
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        value.Enqueue(s_TSerializer.ReadValue(reader));
        ...
        protected override void SerializeImplementation(ref TQueue value, IDataWriter writer)
        {
            try
            {
                ...
                foreach (var element in value)
                {
                    try
                    {
                        s_TSerializer.WriteValue(element, writer);
                    }
                    catch (Exception ex)
                    {
