...
namespace PolymindGames.OdinSerializer
{
    ...
    public sealed class ArrayListFormatter : BaseFormatter<ArrayList>
    {
            ...
            if (entry == EntryType.StartOfArray)
            {
                try
                {
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        value.Add(s_ObjectSerializer.ReadValue(reader));
        ...
        protected override void SerializeImplementation(ref ArrayList value, IDataWriter writer)
        {
            try
            {
                ...
                for (int i = 0; i < value.Count; i++)
                {
                    try
                    {
                        s_ObjectSerializer.WriteValue(value[i], writer);
                    }
                    catch (Exception ex)
                    {
