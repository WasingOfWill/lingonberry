...
namespace PolymindGames.OdinSerializer
{
    ...
    {
        private static readonly Serializer<object> s_ObjectSerializer = Serializer.Get<object>();
        private static readonly Serializer<string> s_StringSerializer = Serializer.Get<string>();
        private static readonly Serializer<Type> s_TypeSerializer = Serializer.Get<Type>();
        private static readonly Serializer<Type[]> s_TypeArraySerializer = Serializer.Get<Type[]>();
        private static readonly Serializer<Delegate[]> s_DelegateArraySerializer = Serializer.Get<Delegate[]>();
        ...
        protected override void DeserializeImplementation(ref T value, IDataReader reader)
        {
            ...
            while ((entry = reader.PeekEntry(out name)) != EntryType.EndOfNode && entry != EntryType.EndOfArray && entry != EntryType.EndOfStream)
            {
                switch (name)
                {
                    case "invocationList":
                        {
                            invocationList = s_DelegateArraySerializer.ReadValue(reader);
                        }
                        break;

                    case "target":
                        {
                            target = s_ObjectSerializer.ReadValue(reader);
                        }
                        break;

                    case "declaringType":
                        {
                            var t = s_TypeSerializer.ReadValue(reader);
                    ...
                    case "methodName":
                        {
                            methodName = s_StringSerializer.ReadValue(reader);
                        }
                        break;
                    ...
                    case "signature":
                        {
                            signature = s_TypeArraySerializer.ReadValue(reader);
                        }
                        break;

                    case "genericArguments":
                        {
                            genericArguments = s_TypeArraySerializer.ReadValue(reader);
                        }
                        break;
        ...
        protected override void SerializeImplementation(ref T value, IDataWriter writer)
        {
            ...
            if (invocationList.Length > 1)
            {
                ...
                s_DelegateArraySerializer.WriteValue("invocationList", invocationList, writer);
                return;
            }
            ...
            if (del.Target != null)
            {
                s_ObjectSerializer.WriteValue("target", del.Target, writer);
            }

            s_TypeSerializer.WriteValue("declaringType", methodInfo.DeclaringType, writer);
            s_StringSerializer.WriteValue("methodName", methodInfo.Name, writer);
            s_TypeSerializer.WriteValue("delegateType", del.GetType(), writer);
            ...
            s_TypeArraySerializer.WriteValue("signature", signature, writer);

            if (methodInfo.IsGenericMethod)
            {
                Type[] genericArguments = methodInfo.GetGenericArguments();
                s_TypeArraySerializer.WriteValue("genericArguments", genericArguments, writer);
            }
        }
