...
namespace PolymindGames.OdinSerializer
{
    ...
    public sealed class HashSetFormatter<T> : BaseFormatter<HashSet<T>>
    {
        private static readonly Serializer<T> s_TSerializer = Serializer.Get<T>();
            ...
            if (entry == EntryType.StartOfArray)
            {
                try
                {
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        value.Add(s_TSerializer.ReadValue(reader));
        ...
        protected override void SerializeImplementation(ref HashSet<T> value, IDataWriter writer)
        {
            try
            {
                ...
                foreach (T item in value)
                {
                    try
                    {
                        s_TSerializer.WriteValue(item, writer);
                    }
                    catch (Exception ex)
                    {
