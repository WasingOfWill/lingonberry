...
namespace PolymindGames.OdinSerializer
{
    ...
    public sealed class StackFormatter<TStack, TValue> : BaseFormatter<TStack>
        where TStack : Stack<TValue>, new()
    {
        private static readonly Serializer<TValue> s_TSerializer = Serializer.Get<TValue>();
        private static readonly bool s_IsPlainStack = typeof(TStack) == typeof(Stack<TValue>);
            ...
            if (entry == EntryType.StartOfArray)
            {
                try
                {
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        value.Push(s_TSerializer.ReadValue(reader));
            ...
            try
            {
                ...
                using (var listCache = Cache<List<TValue>>.Claim())
                {
                    ...
                    for (int i = list.Count - 1; i >= 0; i--)
                    {
                        try
                        {
                            s_TSerializer.WriteValue(list[i], writer);
                        }
                        catch (Exception ex)
                        {
