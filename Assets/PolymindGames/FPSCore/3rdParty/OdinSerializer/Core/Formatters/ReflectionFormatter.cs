...
namespace PolymindGames.OdinSerializer
{
    ...
    public class ReflectionFormatter<T> : BaseFormatter<T>
    {
        ...
        protected override void DeserializeImplementation(ref T value, IDataReader reader)
        {
            ...
            while ((entryType = reader.PeekEntry(out string name)) != EntryType.EndOfNode && entryType != EntryType.EndOfArray && entryType != EntryType.EndOfStream)
            {
                ...
                if (members.TryGetValue(name, out var member) == false)
                {
