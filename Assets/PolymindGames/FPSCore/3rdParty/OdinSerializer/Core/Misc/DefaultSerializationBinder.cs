...
namespace PolymindGames.OdinSerializer
{
    ...
    public sealed class DefaultSerializationBinder : TwoWaySerializationBinder
    {
        private static readonly object s_AssemblyLookupLock = new();
        private static readonly Dictionary<string, Assembly> s_AssemblyNameLookUp = new();
        private static readonly Dictionary<string, Type> s_CustomTypeNameToTypeBindings = new();

        private static readonly object s_TypetonameLock = new();
        private static readonly Dictionary<Type, string> s_NameMap = new(FastTypeComparer.Instance);

        private static readonly object s_NametotypeLock = new();
        private static readonly Dictionary<string, Type> s_TypeMap = new();

        private static readonly object s_AssemblyRegisterQueueLock = new();
        private static readonly List<Assembly> s_AssembliesQueuedForRegister = new();
        private static readonly List<AssemblyLoadEventArgs> s_AssemblyLoadEventsQueuedForRegister = new();

        static DefaultSerializationBinder()
        {
            AppDomain.CurrentDomain.AssemblyLoad += (sender, args) =>
            {
                lock (s_AssemblyRegisterQueueLock)
                {
                    s_AssemblyLoadEventsQueuedForRegister.Add(args);
                }
            };
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                lock (s_AssemblyRegisterQueueLock)
                {
                    s_AssembliesQueuedForRegister.Add(assembly);
                }
            }
        }
        ...
        private static bool RegisterQueuedAssemblies()
        {
            ...
            lock (s_AssemblyRegisterQueueLock)
            {
                if (s_AssembliesQueuedForRegister.Count > 0)
                {
                    toRegister = s_AssembliesQueuedForRegister.ToArray();
                    s_AssembliesQueuedForRegister.Clear();
                }
            }
        ...
        private static bool RegisterQueuedAssemblyLoadEvents()
        {
            ...
            lock (s_AssemblyRegisterQueueLock)
            {
                if (s_AssemblyLoadEventsQueuedForRegister.Count > 0)
                {
                    toRegister = s_AssemblyLoadEventsQueuedForRegister.ToArray();
                    s_AssemblyLoadEventsQueuedForRegister.Clear();
                }
            }
        ...
        private static void RegisterAssembly(Assembly assembly)
        {
            ...
            {
                if (!s_AssemblyNameLookUp.ContainsKey(name))
                {
                    s_AssemblyNameLookUp.Add(name, assembly);
                    wasAdded = true;
                }
            }
                        ...
                        for (int i = 0; i < customAttributes.Length; i++)
                        {
                            ...
                            if (attr != null && attr.NewType != null)
                            {
                                lock (s_AssemblyLookupLock)
                                {
                                    ...
                                    //{
                                    s_CustomTypeNameToTypeBindings[attr.OldTypeName] = attr.NewType;
                                    //}
                                    //else
                                    //{
        ...
        public override string BindToName(Type type, DebugContext debugContext = null)
        {
            ...
            lock (s_TypetonameLock)
            {
                if (s_NameMap.TryGetValue(type, out result) == false)
                {
        ...
        {
            lock (s_NametotypeLock)
            {
                return s_TypeMap.ContainsKey(typeName);
            }
        }
        ...
        public override Type BindToType(string typeName, DebugContext debugContext = null)
        {
            ...
            lock (s_NametotypeLock)
            {
                if (s_TypeMap.TryGetValue(typeName, out result) == false)
                {
        ...
        private Type ParseTypeName(string typeName, DebugContext debugContext)
        {
            ...
            lock (s_AssemblyLookupLock)
            {
                ...
                if (s_CustomTypeNameToTypeBindings.TryGetValue(typeName, out type))
                {
                    return type;
                }
            }
            ...
            ParseName(typeName, out string typeStr, out string assemblyStr);
            ...
            if (!string.IsNullOrEmpty(typeStr))
            {
                ...
                {
                    // Look for custom defined type name lookups defined with the BindTypeNameToTypeAttribute.
                    if (s_CustomTypeNameToTypeBindings.TryGetValue(typeStr, out type))
                    {
                        return type;
                    }
                }
                ...
                if (assemblyStr != null)
                {
                    lock (s_AssemblyLookupLock)
                    {
                        s_AssemblyNameLookUp.TryGetValue(assemblyStr, out assembly);
                    }
        ...
        private Type ParseGenericAndOrArrayType(string typeName, DebugContext debugContext)
        {
            ...
            if (!TryParseGenericAndOrArrayTypeName(typeName, out string actualTypeName, out bool isGeneric, out var genericArgNames, out bool isArray, out int arrayRank)) return null;
            ...
            for (int i = 0; i < typeName.Length; i++)
            {
                if (typeName[i] == '[')
                {
                    ...
                    else
                    {
                        ...
                        else if (isGeneric && ReadGenericArg(typeName, ref i, out string argName))
                        {
