...
namespace PolymindGames.OdinSerializer
{
    ...
    public static class FormatterLocator
    {
        ...
        private static readonly DoubleLookupDictionary<Type, ISerializationPolicy, IFormatter> TypeFormatterMap = new(FastTypeComparer.Instance, ReferenceEqualityComparer<ISerializationPolicy>.Default);
...
#pragma warning restore 618
...
#if UNITY_EDITOR
            OnLocatedFormatter?.Invoke(result);
        ...
        internal static List<IFormatter> GetAllCompatiblePredefinedFormatters(Type type, ISerializationPolicy policy)
        {
            ...
            for (int i = 0; i < FormatterLocators.Count; i++)
            {
                try
                {
                    if (FormatterLocators[i].LocatorInstance.TryGetFormatter(type, FormatterLocationStep.BeforeRegisteredFormatters, policy, out var result))
                    {
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            for (int i = 0; i < FormatterLocators.Count; i++)
            {
                try
                {
                    if (FormatterLocators[i].LocatorInstance.TryGetFormatter(type, FormatterLocationStep.AfterRegisteredFormatters, policy, out var result))
                    {
                        formatters.Add(result);
                    }
                }
